...
def get_enroll_embeddings(embedding_dir):
    for f in os.listdir(embedding_dir):
        spk = f.replace('.pth','')
        # Select the speakers who are in the 'enroll_spk_list'
        if spk in enroll_spk_list:
            embedding_path = os.path.join(embedding_subdir, f)
...
def main():
    ...
    # Get embeddings
    embeddings = get_enroll_embeddings(embedding_dir)
